      ...
      <unicloud-db ref="udb" :collection="collectionList" :where="where" page-data="replace"
        ...
        <uni-table ref="table" :loading="loading" :emptyText="error.message || '没有更多数据'" border stripe type="selection" @selection-change="selectionChange">
          <uni-tr>
            ...
            <uni-th align="center" sortable @sort-change="sortChange($event, 'cover')">封面图</uni-th>
...
<script>
  import { enumConverter, filterToWhere } from '../../js_sdk/validator/news.js';
  ...
  export default {
    data() {
      return {
        collectionList: [ db.collection('news').field('category_id,title,content,excerpt,cover,publish_date,last_modify_date').getTemp(),db.collection('category').field('name as text, _id').getTemp() ],
        query: '',
        where: '',
        orderby: dbOrderBy,
        orderByFieldName: "",
